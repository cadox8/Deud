package es.cadox8.deud.entities.statics.door;
...
public class Door extends StaticEntity {
    ...
    public Door(@NonNull GameAPI gameAPI, float x, float y, String map) {
        super("1737b269-757f-4189-8ae2-6db7d25221c9", "Door", EntityType.DOOR, gameAPI, x, y, Tile.TILEWIDTH, Tile.TILEHEIGHT);
    ...
    public void changeWorld(Player p) {
        if (!canOpen(p)) {
            ((GameState)getGameAPI().getGame().getGameState()).setDialog(new es.cadox8.deud.ux.dialog.Dialog(getGameAPI(), p).addText(Arrays.asList("You need " + Item.get(neededItem).getName() + " to open this")));
            return;
        }
